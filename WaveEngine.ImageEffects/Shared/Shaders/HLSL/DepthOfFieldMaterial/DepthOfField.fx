...
cbuffer Parameters : register(b1)
{
	...
	float FocusDistance			: packoffset(c0.z);
	float FocusRange			: packoffset(c0.w);
	float NearPlane				: packoffset(c1.x);
	float FarParam				: packoffset(c1.y);
...
// Down sampler
float4 psDown(PS_IN_TEXTURE input) : SV_Target0
{
	float4 color = DiffuseTexture.Sample(DiffuseTextureSampler, input.TexCoord);
	float3 result = color.rgb * color.a;

	return float4(result.xyz, 1.0);
}

// Blur
VS_OUT_TEXTURE vsBlur(VS_IN_TEXTURE input)
{
...
float4 psBlur(PS_IN_TEXTUREWEIGTH input) : SV_Target0
{
...
float4 psCombine(PS_IN_TEXTURE input) : SV_Target0
{
	...
	float3 color = lerp(scene, blur, blurFactor);
