...
cbuffer Parameters : register(b1)
{
	...
	float GlowScale				: packoffset(c0.z);
...
VS_OUT_TEXTURE vsBlur(VS_IN_TEXTURE input)
{
	...
	float2 Scale = 0.66 * GlowScale * 2.0 * TexcoordOffset.xy;
...
float4 psUpCombine(PS_IN_TEXTURE input) : SV_Target0
{
	...
	float3 glow = DiffuseTexture1.Sample(DiffuseTextureSampler1, input.TexCoord).rgb;
