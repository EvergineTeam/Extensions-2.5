...
{
    /// <summary>
    /// Provides an abstraction to track changes on a <see cref="Vector2"/> property contained on a
    ...
    public abstract class NetworkVector2PropertySync<K> : NetworkPropertySync<K, Vector2>
        where K : struct, IConvertible
    {
        ...
        {
            return propertiesTable.GetVector2(this.propertyKey);
        }
